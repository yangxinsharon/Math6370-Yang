...
int GramSchmidt2d(vec2d_b X[], int numvectors);
...
int main(int argc, char* argv[]) {
  ...
  // std::cout << sizeof(dat[0][0]) <<"dat"<<std::endl;
  // std::cout << typeid(dat).name() << std::endl;
  a.Write("a_data");
  ...
  // create a few vecs of size 10x3
  ...
  for (int i=0; i<10; i++) {
    for (int j=0; j<3; j++) {
      ...
      X[1][i] = -5.0 + 1.0*(j+3*i);
      X[2][i] = 2.0 + 2.0*(j+3*i);
      X[3][i] = 20.0 - 1.0*(j+3*i);
      X[4][i] = -20.0 + 1.0*(j+3*i);
    }
  }
  ...
  std::cout << "Testing TwoNorm (should be xx):  " << TwoNorm(b) << std::endl;

  std::cout << "Testing RmsNorm (should be xx):  " << RmsNorm(c) << std::endl;
  ...
  std::cout << "Testing Dot (should be xx):  " << Dot(a, c) << std::endl;
