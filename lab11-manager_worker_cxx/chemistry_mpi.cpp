/* Daniel R. Reynolds; Sharon Yang
   SMU Mathematics
   Math 4370 / 6370 */

// Inclusions
#include <stdlib.h>
#include <iostream>
#include <cmath>
#include "mpi.h"

// Prototypes
void chem_solver(double, double*, double*, double*,
		 double, double, int, int*, double*);


/* Example routine to compute the equilibrium chemical densities at
   a number of spatial locations, given a (random) background temperature
   field.  The chemical rate equations and solution strategy are in the
   subroutine chem_solver, which is called at every spatial location. */
int main(int argc, char* argv[]) {
  
  // declarations
  double *Pbuf, *Sbuf;
  bool more_work;
  int tag, numsent;
  MPI_Status status;

  // 1. set solver input parameters
  const int maxit = 1000000;
  const double lam = 1.e-2;
  const double eps = 1.e-10;

  // initialize MPI
  int ierr = MPI_Init(&argc, &argv);
  if (ierr != MPI_SUCCESS) {
     std::cerr << "Error in calling MPI_Init\n";
     return 1;
  }
  ierr = MPI_Comm_size(MPI_COMM_WORLD, &numprocs);
  if (ierr != MPI_SUCCESS) {
     std::cerr << "Error in calling MPI_Comm_size\n";
     return 1;
  }
  ierr = MPI_Comm_rank(MPI_COMM_WORLD, &myid);
  if (ierr != MPI_SUCCESS) {
     std::cerr << "Error in calling MPI_Comm_rank\n";
     return 1;
  }

  // manager
  if (myid == 0) {

    // 2. input the number of intervals
    int n;
    std::cout << "Enter the number of intervals (0 quits):\n";
    std::cin >> n;
    if (n < 1) {
      return 1;
    }
  
    // 3. allocate temperature and solution arrays
    double *T = new double[n];
    double *u = new double[n];
    double *v = new double[n];
    double *w = new double[n];
  
    // 4. set random temperature field, initial guesses at chemical densities
    for (int i=0; i<n; i++)  T[i] = random() / (pow(2.0,31.0) - 1.0);
    for (int i=0; i<n; i++)  u[i] = 0.35;
    for (int i=0; i<n; i++)  v[i] = 0.1;
    for (int i=0; i<n; i++)  w[i] = 0.5;

    // 5. start timer
    double stime = MPI_Wtime();
    
    numsent = 0;

    for (int i=0; i<numprocs-1; i++) {
      if (numsent + 1 <= n)
        for (int i=0; i<1; i++) Pbuf[i] = T[numsent+i];
      else {
        for (int i=0; i<n-numsent; i++) Pbuf[i] = T[numsent+i];
        for (int i=n=numsent; i<1; i++) Pbuf[i] = 1.0;
      }
      // send with tag as entry in temperature array
      ierr = MPI_Send(Pbuf, 1, MPI_DOUBLE, i+1, i, MPI_COMM_WORLD);
      if (ierr != 0) {
        printf("Error in MPI_Send = %i\n",ierr);
        ierr = MPI_Abort(MPI_COMM_WORLD, 1);
        return 1;
      }
      numsent += 1;
    } // for i


    // if all work was done in first phase, receive work only
    if (numsent >= n-1) {

      // receive work from each proc
      for (i=1; i<numprocs; i++) {

        // receive answers from any processor 
        ierr = MPI_Recv(Sbuf, 3, MPI_DOUBLE, MPI_ANY_SOURCE, 
            MPI_ANY_TAG, MPI_COMM_WORLD, &status);
        if (ierr != 0) {
          printf("Error in MPI_Recv = %i\n",ierr);
          ierr = MPI_Abort(MPI_COMM_WORLD, 1);
          return 1;
        }
        // decode the sender and solution entry from status 
        sender   = status.MPI_SOURCE;
        ansentry = status.MPI_TAG;
        // store results 
        if (ansentry+1-1 <= n) {
          for (i=0; i<1; i++)  u[ansentry+i] = Sbuf[0];
          for (i=0; i<1; i++)  v[ansentry+i] = Sbuf[1];
          for (i=0; i<1; i++)  w[ansentry+i] = Sbuf[2];
        } else {
          for (i=0; i<n-ansentry+1; i++)  u[ansentry+i] = Sbuf[0];
          for (i=0; i<n-ansentry+1; i++)  v[ansentry+i] = Sbuf[1];
          for (i=0; i<n-ansentry+1; i++)  w[ansentry+i] = Sbuf[2];
        }
  
        // tell senders that work is complete by sending message of zero size
        ierr = MPI_Send(Pbuf, 0, MPI_DOUBLE, sender, 0, MPI_COMM_WORLD);
        if (ierr != 0) {
          printf("Error in MPI_Send = %i\n",ierr);
          ierr = MPI_Abort(MPI_COMM_WORLD, 1);
          return 1;
        }
        
            } // end for i 
            
          } else {  
      
            // more work left; receive work from each proc, assign more if available
            for (int i=1; i<=ceil(1.0*n/1); i++) {
        
              // receive answers from any processor 
              ierr = MPI_Recv(Sbuf, 3, MPI_DOUBLE, MPI_ANY_SOURCE, 
                  MPI_ANY_TAG, MPI_COMM_WORLD, &status);
              if (ierr != 0) {
                printf("Error in MPI_Recv = %i\n",ierr);
                ierr = MPI_Abort(MPI_COMM_WORLD, 1);
                return 1;
              }
              // decode the sender and solution entry from status 
              sender   = status.MPI_SOURCE;
              ansentry = status.MPI_TAG;
              // store results 
              if (ansentry+chunk-1 <= n) {
                for (int i=0; i<chunk; i++)  u[ansentry+i] = Sbuf[0];
                for (int i=0; i<chunk; i++)  v[ansentry+i] = Sbuf[1];
                for (int i=0; i<chunk; i++)  w[ansentry+i] = Sbuf[2];
              } else {
                for (int i=0; i<n-ansentry+1; i++)  u[ansentry+i] = Sbuf[0];
                for (int i=0; i<n-ansentry+1; i++)  v[ansentry+i] = Sbuf[1];
                for (int i=0; i<n-ansentry+1; i++)  w[ansentry+i] = Sbuf[2];
              }
              if (numsent < n) {  // send another set of rows, if available 
                if (numsent + 1 <= n)
                  for (i=0; i<1; i++) Sbuf[i] = T[numsent+i];
                else {
                  for (i=0; i<n-numsent; i++) Sbuf[i] = T[numsent+i];
                  for (i=n=numsent; i<chunk; i++) Sbuf[i] = 1.0;
                }
                ierr = MPI_Send(Sbuf, 1, MPI_DOUBLE, sender, numsent, 
                    MPI_COMM_WORLD);
                if (ierr != 0) {
                  printf("Error in MPI_Send = %i\n",ierr);
                  ierr = MPI_Abort(MPI_COMM_WORLD, 1);
                  return 1;
                }
                numsent += 1;
              } else {  // tell senders that work is complete by sending message of zero size
                ierr = MPI_Send(Sbuf, 0, MPI_DOUBLE, sender, 0, MPI_COMM_WORLD);
                if (ierr != 0) {
                  printf("Error in MPI_Send = %i\n",ierr);
                  ierr = MPI_Abort(MPI_COMM_WORLD, 1);
                  return 1;
                }
              } // if numsent 
                       
                  } // for i 
            
                } // if numsent 

    // 7. stop timer
    double ftime = MPI_Wtime();
    double runtime = ftime - stime;
  
    // 8. output solution time
    std::cout << "     runtime = " << runtime << std::endl;
  
    // 9. free temperature and solution arrays
    delete[] T;
    delete[] u;
    delete[] v;
    delete[] w;



  // worker
  } else {

    // set up workers' own local variables
    double *T = new double[1];
    double *u = new double[1];
    double *v = new double[1];
    double *w = new double[1];
  
    // 6. call solver over n intervals
    // for (int i=0; i<n; i++) {
    // while loop
    more_work = true;
    while (more_work) {
      ierr = MPI_Recv(Pbuf, 4, MPI_DOUBLE, 0, MPI_ANY_TAG, MPI_COMM_WORLD, &status);
      int its;
      double res;
      tag = status.MPI_TAG;
      if (tag == 0) {
        more_work = false;
      } else{
        for (int i=0; i<1; i++)  T[i] = Pbuf[0];
        for (int i=0; i<1; i++)  u[i] = Pbuf[1];
        for (int i=0; i<1; i++)  v[i] = Pbuf[2];
        for (int i=0; i<1; i++)  w[i] = Pbuf[3];
        for (int i=0; i<1; i++) {
          chem_solver(T[i], &(u[i]), &(v[i]), &(w[i]), lam, eps, maxit, &its, &res);
          if (res < eps)
            std::cout << "    i = " << i << "  its = " << its << std::endl;
          else {
            std::cout << "    error: i=" << i << ", its=%i" << its << ", res=" << res
            << ", u=" << u[i] << ", v=" << v[i] << ", w=" << w[i] << std::endl;
            ierr = MPI_Abort(MPI_COMM_WORLD, 1);
            return 1;
          }
        }
        for (int i=0; i<1; i++)  Sbuf[0] = u[i];
        for (int i=0; i<1; i++)  Sbuf[1] = v[i];
        for (int i=0; i<1; i++)  Sbuf[2] = w[i];
        ierr = MPI_Send(Sbuf, 3, MPI_DOUBLE, 0, tag, MPI_COMM_WORLD);
      }
    }
  }




  // finalize MPI
  ierr = MPI_Finalize();

  return 0;
} // end main
