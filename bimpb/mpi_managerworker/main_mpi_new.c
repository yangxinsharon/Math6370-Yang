...
int main(int argc, char *argv[]) {
   ...
   double stime, ftime, runtime;
	...
	h=(double *) calloc (ldh*(RESTRT+2), sizeof(double));
  	...
  	if (myid == 0) { 
   	...
   	// sprintf(density,"%s",argv[2]);
		...
		// /* determine each processor's interval */
	...
	// worker code
    	...
    	double locN, locbvct, locxvct, locwork, *locmatvec, *psolve????;
		...
		void matvecmul(const double *x, double *y, double *q, int nface, double *tr_xyz, double *tr_q, double *tr_area, double alpha, double beta)
		gmres_(&N, bvct, xvct, &RESTRT, work, &ldw, h, &ldh, &iter, &resid, &matvec, &psolve, &info);
