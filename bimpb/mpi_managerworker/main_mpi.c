...
int main(int argc, char *argv[]) {
   ...
   /* MPI variables */
   int ierr, numprocs, myid, iend, numsent;
   double stime, ftime, runtime;
   MPI_Status status;
 	...
 	if (myid == 0) { 
   	...
   	// sprintf(density,"%s",argv[2]);
		...
		// MPI_Send() to comp_source: atmchr, chrpos, tr_xyz, tr_q, nface, nchr
		comp_source_wrapper(); //wraps the solvation energy computation

		// MPI_Recv() bvct from comp_source

		N=2*nface;

  		/* broadcast bvct to all procs */
  		ierr = MPI_Bcast(bvct, N, MPI_DOUBLE, 0, MPI_COMM_WORLD);
	  	if (ierr != MPI_SUCCESS) {
	   ...
	   // MPI_Send() xvct for gmres

	   // MPI_Recv() y from gmres 


		// MPI_Send() final potential
		// MPI_Recv() energy
		soleng=0.0;
  	...
  	// worker code
  		...
  		// MPI_Recv() comp_source
  		// MPI_Send() comp_source results

  		// MPI_Recv() x0 for GMRES
